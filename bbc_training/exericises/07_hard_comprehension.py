...
def comprehension(option):
  if option == '1':
    ...
    for individual in individuals:
      theSum += sum([specie['legs'] for specie in species if specie['name'] == individual['species']])

    return theSum

  if option == '2':
    return sum([spec['legs'] for individual in individuals for spec in species if spec['name'] == individual['species']])
